...
| [`front-plugin-js/order.html`](../front-plugin-js/order.html) | Sends `session.claim` on enter (DISPATCHED → IN_PROGRESS). Renders the custom 메디캐시 selection UI (the medicash-selection render block introduced by commit `7f98300`). On medicash choice: shows `sdk.template.renderOrderPage` as a confirmation screen — backend pre-populates `orderSnapshot.summary.earned` (label "이번 결제 적립", suffix "캐시") for the expected-accrual line; plugin appends "메디캐시 사용" as a discount and overrides `summary.totalAmount` to the net out-of-pocket amount. On `onClick`: sends `session.chargeContext`. If `chargedSupplyValue + chargedTax > 0` (card path) the plugin stays on the renderOrderPage screen — Toss firmware will auto-overlay the 통합결제창. If `charged === 0` (100% 메디캐시) the plugin navigates to `payment.html` for the success-only branch. Also handles `session.abort(USER_BACKED_OUT)` when the user taps back during the medicash UI or via the renderOrderPage `onBack` callback. | The `renderWaitingScreen` function in `order.html`; the `AWAITS_PROCEED` feature gate around the waiting-screen render; the `session.proceed` case in the WS B message handler; the `waitingForProceed` state variable; the 120s defensive timeout inside the waiting screen. |
...
- `sdk.template.renderOrderPage` — confirmation screen after the custom 메디캐시 selection in `order.html`. Backend populates `orderSnapshot.summary.earned` so the 이번 결제 적립 line renders natively; plugin appends "메디캐시 사용" to `discounts` and overrides `summary.totalAmount` to the net out-of-pocket amount. `session.chargeContext` is sent from this screen's `onClick`. `onBack` is wired best-effort (Toss SDK `[GAP]`).
...
### 7.8 메디캐시 deduction failure on SUCCEEDED (out of plugin scope)

Per backend agreement (2026-05-13), 메디캐시 deduction is part of the synchronous SUCCEEDED commit transaction. If deduction fails, BE rolls the entire transaction back — session stays at `IN_PROGRESS`, an `error` frame is sent to plugin and CRM per §13 scope rule (toss-payment-flow.md), and the session expires via the BE-owned IN_PROGRESS timer.

**Plugin does not handle this error frame.** [`payment.html`](../front-plugin-js/payment.html) closes its WS shortly after sending `session.result(tossResponse: null)`, so the error frame is typically missed in this narrow window — that's by design. Reconciliation when this happens is out-of-band ops, not a plugin contract concern.

(Supersedes the prior async-retry / `MEDICASH_DEDUCT_PENDING` design.)
