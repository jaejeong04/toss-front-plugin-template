...
- **Give-up on the payment page now rests solely on `renderOrderResultPage.onBack`, which is a `[GAP]`** — the frontend spec lists it as undocumented. The deleted screen used `renderSelectPage`, whose `onBack` *is* documented, and it re-appeared on every retry cycle, so ≥5만원 payments previously always had a documented give-up surface. They no longer do. If `renderOrderResultPage.onBack` does not fire on device, an abandoned failure screen has no terminal path at all and resolves only via the `EXPIRED` watchdog. **Device-verify before this ships to a live clinic.**
...
- **⚠️ The `installment` value domain is now firmware-defined.** The pre-2026-07-22 plugin screen offered exactly `0` (일시불) + `2`–`12`; that bound is **gone**. Whatever the module's picker offers is what arrives on `response.card.installment` — possibly `1`, possibly 18/24개월 for 무이자 events. **BE and CRM must not validate, constrain, or truncate the value** — persist and forward it verbatim.
...
- **[BE]** C4 `IN_PROGRESS`→`EXPIRED` watchdog: 할부 dwell is **expected** to sit inside the firmware's own `timeoutMs` window rather than being separate pre-attempt plugin think-time — but that is an inference from the 2026-07-22 observation, **not** a measurement, and it *narrows* a safety margin. Measure it ([MUST TEST] #6) before shrinking the timer. Failure-screen dwell and N retries still apply.
- **[MUST TEST]** (1) **[SHIP-BLOCKING]** omitting `installment` still shows the firmware 할부 row at ≥5만원 and not below — this is the load-bearing assumption of the whole removal; if it fails, 할부 is impossible at the kiosk with no plugin fallback; (2) a 3개월 selection echoes back as `response.card.installment === 3`; (3) installment approval end-to-end; (4) cancel of an installment approval with vs. without `cancelParams.installment`; (5) overlay transition (blank-WebView family); (6) whether `timeoutMs` bounds the 서명/할부 dwell — time a 6만원 attempt with a deliberate ~60s dwell on the 개월수 picker and see whether `requestPayment` times out; (7) **[SHIP-BLOCKING]** `renderOrderResultPage.onBack` actually fires on device (give-up now depends on it alone).
